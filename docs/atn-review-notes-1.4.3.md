# Reviewer notes for version 1.4.3

The file submitted to addons.thunderbird.net is:

`dist/thunderbird-ios-imap-notes-1.4.3.xpi`

This is the **Standard** edition. It contains no `experiment_apis` manifest
entry, no `experiments/` directory, no call to `browser.notesHeader` and no
custom `update_url`. Updates for this edition are provided exclusively by
addons.thunderbird.net.

The separate `-header-controls.xpi` file published on GitHub is not submitted
to addons.thunderbird.net. It has its own GitHub update channel and cannot cause
the submitted Standard edition to download code from GitHub.
...
the Standard edition uses exclusively Thunderbird's built-in MailExtension
APIs. The optional Header Controls edition remains available from GitHub for
users who deliberately install it outside the add-on store.
...
There is no remote executable code and the Standard edition does not transmit
note content to a third-party service. Network access is performed by
Thunderbird's own account and IMAP implementation.
...
npm run check
npm run build:xpi
unzip -t dist/thunderbird-ios-imap-notes-1.4.3.xpi
unzip -p dist/thunderbird-ios-imap-notes-1.4.3.xpi manifest.json
unzip -Z1 dist/thunderbird-ios-imap-notes-1.4.3.xpi | grep '^experiments/'
```
